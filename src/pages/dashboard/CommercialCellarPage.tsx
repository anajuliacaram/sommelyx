  ...
  return (
    <div className="min-h-screen bg-[#F4F1EC] text-[#1A1A1A]">
      ...
      {isLoading ? (
        ...
        <div className={cn("grid gap-4", showLabels ? "grid-cols-2 sm:grid-cols-2 xl:grid-cols-3" : "grid-cols-2 sm:[grid-template-columns:repeat(auto-fill,minmax(260px,1fr))]") }>
          {filtered.map((wine) => (
            <WineCard
              key={wine.id}
              wine={wine}
